...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MultiCollisionMovementComponent)

UMultiCollisionMovementComponent::UMultiCollisionMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UMultiCollisionMovementComponent::UpdateAdditionalUpdatedComponents()
{
	...
	MultiCollisionOwner->ForEachComponent<UMultiCollisionCapsuleComponent>(false, [&](auto AdditionalComponent)
	{
...
bool UMultiCollisionMovementComponent::MoveUpdatedComponentImpl(const FVector& Delta, const FQuat& Rotation, bool bSweep, FHitResult* OutHit, ETeleportType Teleport)
{
	...
	constexpr float MinMovementDistSq = FMath::Square(4.f * KINDA_SMALL_NUMBER);
	...
	UpdatedComponent->MoveComponent(NewDelta, NewRotation, bSweep, nullptr, MoveComponentFlags, Teleport);
