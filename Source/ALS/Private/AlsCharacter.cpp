...
void AAlsCharacter::RefreshPhysicalAnimation(float DeltaTime)
{
	...
	else if (LocomotionAction == AlsLocomotionActionTags::Ragdolling)
	{
		if (PhysicalAnimationState.Current != EAlsPhysicalAnimationProfile::Ragdoll)
		{
			...
			{
				GetMesh()->SetConstraintProfileForAll(UAlsConstants::RagdollPAProfileName(), true);
			}
		}
	...
	else
	{
		...
		if (OverlayMode == AlsOverlayModeTags::Injured)
		{
			if (PhysicalAnimationState.Current != EAlsPhysicalAnimationProfile::Injured)
			{
				...
				if(Settings->PhysicalAnimation.bUseConstraintProfile)
				{
					GetMesh()->SetConstraintProfileForAll(UAlsConstants::InjuredPAProfileName(), true);
				}
			}
		...
		else
		{
			if (PhysicalAnimationState.Current != EAlsPhysicalAnimationProfile::Default)
			{
				...
				if(Settings->PhysicalAnimation.bUseConstraintProfile)
				{
					GetMesh()->SetConstraintProfileForAll(UAlsConstants::DefaultPAProfileName(), true);
				}
			}
		}
		if (!LocomotionAction.IsValid())
		{
			ActiveParts |= EAlsPhysicalAnimationPartMask::BelowTorso;
			if (!Settings->PhysicalAnimation.bIsAffectedClavicle)
			{
				ActiveParts &= ~(EAlsPhysicalAnimationPartMask::LeftClavicle | EAlsPhysicalAnimationPartMask::RightClavicle);
			}
		...
		if (bChangeAny)
		{
			for (int i = 0; i < EAlsPhysicalAnimationPart::MAX_NUM; ++i)
			{
				...
				case EAlsPhysicalAnimationPart::Neck:
					...
					break;
				case EAlsPhysicalAnimationPart::LeftClavicle:
					BoneName = UAlsConstants::ClavicleLeftBoneName();
					break;
				case EAlsPhysicalAnimationPart::RightClavicle:
					BoneName = UAlsConstants::ClavicleRightBoneName();
					break;
